...
// define our link functions
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      tep_redirect(tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $HTTP_GET_VARS['cPath'] . '&lID=' . $HTTP_GET_VARS['lID']));
      break;
...
<?php
    ...
    if (isset($HTTP_GET_VARS['search'])) {
      ...
      $link_categories_query = tep_db_query("select l.link_categories_id, ld.link_categories_name, ld.link_categories_name, l.link_categories_status, l.link_categories_image, l.parent_id, l.link_categories_sort_order, l.link_categories_date_added, l.link_categories_last_modified from " . TABLE_LINK_CATEGORIES . " l, " . TABLE_LINK_CATEGORIES_DESCRIPTION . " ld where l.link_categories_id = ld.link_categories_id and ld.language_id = '" . (int)$languages_id . "' and ld.link_categories_name like '%" . tep_db_input($search) . "%' order by " . $order);
    } else {
      $link_categories_query = tep_db_query("select l.link_categories_id, ld.link_categories_name, ld.link_categories_name, l.link_categories_status, l.link_categories_image, l.parent_id, l.link_categories_sort_order, l.link_categories_date_added, l.link_categories_last_modified from " . TABLE_LINK_CATEGORIES . " l, " . TABLE_LINK_CATEGORIES_DESCRIPTION . " ld where l.parent_id = '" . (int)$current_category_id . "' and l.link_categories_id = ld.link_categories_id and ld.language_id = '" . (int)$languages_id . "' order by " . $order);
    }
...
// Get parent_id for sublink_categories if search
      if (isset($HTTP_GET_VARS['search'])) $cPath= $link_categories['parent_id'];

      if ((!isset($HTTP_GET_VARS['lID']) && !isset($HTTP_GET_VARS['lID']) || (isset($HTTP_GET_VARS['lID']) && ($HTTP_GET_VARS['lID'] == $link_categories['link_categories_id']))) && !isset($cInfo) && (substr($action, 0, 3) != 'new')) {
  ...
  ?>
                ...
                <?php 
                 if ($catCount)
                   echo '<a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, tep_get_path($link_categories['link_categories_id'])) . '">' . tep_image(DIR_WS_ICONS . 'folder.gif', ICON_FOLDER) . '</a>&nbsp;<b>' . $link_categories['link_categories_name'] . '</b>&nbsp;( ' . $catCount . ' )';
                 else   
                   echo '<a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, tep_get_path($link_categories['link_categories_id'])) . '">' . tep_image(DIR_WS_ICONS . 'folder.gif', ICON_FOLDER) . '</a>&nbsp;<b>' . $link_categories['link_categories_name'] . '</b>&nbsp;'; 
                ?>
                </td>
                ...
                if ($link_categories['link_categories_status'] == STATUS_CATEGORIES_ENABLE_FLAG) {
                  ...
                  tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . 
                  tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
                }
                ?>            
                </td>
                <td class="dataTableContent" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($link_categories['link_categories_id'] == $cInfo->link_categories_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&lID=' . $link_categories['link_categories_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
              ...
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php if (sizeof($cPath_array) > 0) echo '<a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, $cPath_back . 'lID=' . $current_category_id) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>&nbsp;'; if (!isset($HTTP_GET_VARS['search'])) echo '<a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&action=new_category') . '">' . tep_image_button('button_new_category.gif', IMAGE_NEW_CATEGORY) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&action=new_link') . '">' . '</a>'; ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
<?php
    ...
    switch ($action) {
      case 'new_category':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      case 'edit_category':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&lID=' . $cInfo->link_categories_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      case 'delete_category':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&lID=' . $cInfo->link_categories_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      case 'move_category':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_move.gif', IMAGE_MOVE) . ' <a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&lID=' . $cInfo->link_categories_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      ...
      default:
        if ($rows > 0) {
          if (isset($cInfo) && is_object($cInfo)) { // category info box contents
            ...
            $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&lID=' . $cInfo->link_categories_id . '&action=edit_category') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&lID=' . $cInfo->link_categories_id . '&action=delete_category') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_LINK_CATEGORIES, 'cPath=' . $cPath . '&lID=' . $cInfo->link_categories_id . '&action=move_category') . '">' . tep_image_button('button_move.gif', IMAGE_MOVE) . '</a>');
