...
// Return the links url, based on whether click count is turned on/off
  ...
  function tep_get_link_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false) {
    ...
    if ($include_itself) {
      $category_query = tep_db_query("select cd.link_categories_name from " . TABLE_LINK_CATEGORIES_DESCRIPTION . " cd where cd.language_id = '" . (int)$languages_id . "' and cd.link_categories_id = '" . (int)$parent_id . "'");
      $category = tep_db_fetch_array($category_query);
