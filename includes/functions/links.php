...
// TABLES: links_to_link_categories
  ...
  function tep_get_parent_category($catID = '0') {
    ...
    $categories_query = tep_db_query("select c.parent_id from " . TABLE_LINK_CATEGORIES . " c, " . TABLE_LINK_CATEGORIES_DESCRIPTION . " cd where c.link_categories_id = '" . $catID . "' and c.link_categories_id = cd.link_categories_id and cd.language_id = '" . (int)$languages_id . "' LIMIT 1");
  ...
  function tep_get_sublink_categories($parent_id = '0', $exclude = '', $category_subcats = '') {
    ...
    $categories_query = tep_db_query("select c.link_categories_id, cd.link_categories_name, c.parent_id from " . TABLE_LINK_CATEGORIES . " c, " . TABLE_LINK_CATEGORIES_DESCRIPTION . " cd where c.link_categories_id = cd.link_categories_id and cd.language_id = '" . (int)$languages_id . "' and c.parent_id = '" . (int)$parent_id . "' order by c.link_categories_sort_order, cd.link_categories_name");
...
// Search the given page for the given phase(s)  
 ...
 function tep_get_child_categories($parentID) {
    ...
    $categories_query = tep_db_query("select lc.link_categories_id, lcd.link_categories_name, lcd.link_categories_description, lc.link_categories_image, lc.parent_id from " . TABLE_LINK_CATEGORIES . " lc, " . TABLE_LINK_CATEGORIES_DESCRIPTION . " lcd where lc.link_categories_id = lcd.link_categories_id and lc.link_categories_status = '1' and lc.parent_id = '" . $parentID  . "' and lcd.language_id = '" . (int)$languages_id . "' order by lc.link_categories_sort_order, lcd.link_categories_name");
...
// TABLES: links, links_to_link_categories, link_categories
  function tep_links_in_category_count($categories_id, $include_deactivated = false) {
    ...
    if ($include_deactivated) {
      $links_query = tep_db_query("select count(*) as total from " . TABLE_LINKS . " l, " . TABLE_LINKS_TO_LINK_CATEGORIES . " l2c where l.links_id = l2c.links_id and l2c.link_categories_id = '" . (int)$categories_id . "'");
    } else {
      $links_query = tep_db_query("select count(*) as total from " . TABLE_LINKS . " l, " . TABLE_LINKS_TO_LINK_CATEGORIES . " l2c where l.links_id = l2c.links_id and l.links_status = '2' and l2c.link_categories_id = '" . (int)$categories_id . "'");
    }
    ...
    if ($from == 'link') {
      ...
      while ($categories = tep_db_fetch_array($categories_query)) {
        if ($categories['categories_id'] == '0') {
          ...
          $category_query = tep_db_query("select ld.link_categories_name, l.parent_id from " . TABLE_LINK_CATEGORIES . " l, " . TABLE_LINK_CATEGORIES_DESCRIPTION . " ld where l.link_categories_id = '" . (int)$categories['link_categories_id'] . "' and l.link_categories_id = ld.link_categories_id and ld.language_id = '" . (int)$languages_id . "'");
          $category = tep_db_fetch_array($category_query);
      ...
      $category_query = tep_db_query("select ld.link_categories_name, l.parent_id from " . TABLE_LINK_CATEGORIES . " l, " . TABLE_LINK_CATEGORIES_DESCRIPTION . " ld where l.link_categories_id = '" . (int)$id . "' and l.link_categories_id = ld.link_categories_id and ld.language_id = '" . (int)$languages_id . "'");
      $category = tep_db_fetch_array($category_query);
