...
<!-- left_navigation //-->
<?php require(DIR_WS_INCLUDES . 'column_left_test.php'); ?>
<!-- left_navigation_eof //-->
    </table></td>
<!-- body_text //-->
<?php
...
*/
 echo tep_image(DIR_WS_IMAGES . 'table_background_default.gif', $category['categories_name']); ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
// check to see if there are deeper categories within the current category
      ...
      for($i=0, $n=sizeof($category_links); $i<$n; $i++) {
        $categories_query = tep_db_query("select count(*) as total from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categoris_status = 1, c.parent_id = '" . (int)$category_links[$i] . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "'");
        $categories = tep_db_fetch_array($categories_query);
        if ($categories['total'] < 1) {
          // do nothing, go through the loop
        } else {
          $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_status=1,c.parent_id = '" . (int)$category_links[$i] . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' order by sort_order, cd.categories_name");
...
// We show them all
        $listing_sql = "select " . $select_column_list . " p.products_id, p.manufacturers_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from " . TABLE_PRODUCTS . " p left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_MANUFACTURERS . " m , " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c left join " . TABLE_CATEGORIES . " c on c.categories_id = p2c.categories_id where c.categories_status=1 and  p.products_status = '1' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "' and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = '" . (int)$HTTP_GET_VARS['manufacturers_id'] . "'";
      }
    } else {
...
// Get the right image for the top-right
    $image = DIR_WS_IMAGES . 'table_background_default.gif';
/*
    if (isset($HTTP_GET_VARS['manufacturers_id'])) {
...
*/
?>
            <td align="right"><?php echo tep_image($image, HEADING_TITLE); ?></td>
          </tr>
          <tr><td colspan="2" class="main"><strong class="product-listing-product-name">Klik op een foto voor meerdere impressies</strong></td></tr>
        </table></td>
      </tr>
      <tr>
