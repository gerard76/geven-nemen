...
// IDEAL AANPASSING
  if(tep_session_is_registered('paymentid'))
  		if(tep_db_num_rows(tep_db_query("SELECT * FROM ".TABLE_MOLLIEIDEAL_PAYMENTS." WHERE payment_id='$paymentid'"))==0)
		{
  ...
  if (is_object($$payment)) {
    ...
    if ($payment_class->code == 'idealmolliem') {
      if ((!tep_session_is_registered('trans')) || ($trans != 'success'))
        $do_email = false;
    }
  }
if ((tep_session_is_registered('paymentid')) && ($payment_class->code != 'idealmolliem')) {
   tep_db_query("UPDATE ".TABLE_MOLLIEIDEAL_PAYMENTS." SET payment_status='".MODULE_PAYMENT_IDEALMOLLIEM_ORDER_NOT_PAID_STATUS_ID."', date_last_check=now() WHERE payment_id='" . $paymentid."'");
   tep_session_unregister('paymentid');
   tep_session_unregister('trans');
}
if (!tep_session_is_registered('paymentid') AND !$_GET['transaction_id']) {   
...
if (($do_email) && ($payment_class->code != 'idealmolliem')) {
...
// IDEAL AANPASSING
  ...
  $orderid = tep_db_query("SELECT order_id FROM " . TABLE_MOLLIEIDEAL_PAYMENTS . " WHERE payment_id='" . $paymentid . "'");    
...
/* IDEAAL/IDEAL AANPASSING */
  ...
  } elseif  ((($payment_class->code == 'idealmolliem'))  && (($trans == 'pending') || ($trans == 'success'))) {
    tep_session_unregister('paymentid');
    tep_session_unregister('trans');
